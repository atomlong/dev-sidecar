...
### Added
- Added `observatoryProbeUrl` config option in `packages/core/src/modules/plugin/xray/config.js` for Stage1/observatory runtime probing, separate from the existing `probeUrl` (Stage3 cache probing). When set, the live `config.json` observatory uses this URL instead of `probeUrl`; when empty, falls back to `probeUrl`. This lets operators use a stricter probe target (e.g. `https://chatgpt.com/`) for runtime node selection while keeping the lenient `gstatic.com/generate_204` for Stage3 broad-brush cache probing. xray observatory counts any HTTP response (including 403) as success; only timeout/reset counts as failure. A node that returns 403 on `chatgpt.com` (CF bot protection) is marked available, while a broken HTTP proxy that times out is marked unavailable and excluded from the balancer. Three `genConfig()` call sites in `packages/core/src/modules/plugin/xray/index.js` were updated to pass `cfg.observatoryProbeUrl || cfg.probeUrl`.
