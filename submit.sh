...
cleanup() {
    ...
    if [ -n "$BRANCH_PRIVATE" ] && [ "$current_branch" != "$BRANCH_PRIVATE" ]; then
        ...
        # Restore submodules after switching back to private branch
        if [ -f ".gitmodules" ]; then
            log_info "Restoring submodules..."
            git submodule update --init --recursive 2>/dev/null || log_warn "Submodule restore failed. Run 'git submodule update --init --recursive' manually."
        fi
    fi
    exit $exit_code
}
...
# Description: Find the best parent branch for creating a new feature branch based on naming convention
...
find_best_parent() {
    ...
    # For dev/xxx branches, prioritize the corresponding feature/xxx branch
    if [[ "$private_branch" == dev/* ]]; then
        local suffix="${private_branch#dev/}"
        local expected_feature="$remote/feature/$suffix"
        
        # Check if the expected feature branch exists
        if git show-ref --verify --quiet "refs/remotes/$expected_feature"; then
            local base=$(git merge-base "$private_branch" "$expected_feature" 2>/dev/null || true)
            if [ -n "$base" ]; then
                echo "$base"
                log_info "Using matching feature branch: $expected_feature (Base: $base)" >&2
                return
            fi
        fi
    fi
    
    # Otherwise, use main/master
    local main_branch=$(get_main_branch_name "$remote")
    local main_ref="$remote/$main_branch"
    if git show-ref --verify --quiet "refs/remotes/$main_ref"; then
        local base=$(git merge-base "$private_branch" "$main_ref" 2>/dev/null || true)
        if [ -n "$base" ]; then
            echo "$base"
            log_info "Using main branch: $main_ref (Base: $base)" >&2
            return
        fi
    fi
...
# Description: Restore private files from a specified source or auto-detect using convention.
#              Convention: Private files always come from 'develop' branch.
...
# Params:
#   $1 - (Optional) Source ref (branch name or commit hash). If empty, uses convention-based detection.
# Return: None
# Exit: 0 on success, 1 on failure
restore_private_files() {
    ...
    # 1. Auto-detection if source_ref is missing (convention-based)
    if [ -z "$source_ref" ]; then
        log_info "No source reference provided. Using convention-based detection..."
        
        # Convention: Private files always come from 'develop' branch
        if git show-ref --verify --quiet "refs/heads/develop"; then
            source_ref="develop"
            log_info "Using local 'develop' branch as private source."
        elif git show-ref --verify --quiet "refs/remotes/origin/develop"; then
            source_ref="origin/develop"
            log_info "Using 'origin/develop' as private source."
        else
            # Fallback: try to find any dev/* branch (excluding current branch)
            local other_dev
            other_dev=$(git branch -a | grep -E "(^[[:space:]]+|/)dev/" | grep -v "$CURRENT_BRANCH" | head -1 | sed 's/^[[:space:]]*//' | sed 's/^remotes\///')
            if [ -n "$other_dev" ]; then
                source_ref="$other_dev"
                log_warn "No 'develop' branch found. Using '$source_ref' as fallback."
            else
                log_error "Cannot determine private file source."
                log_error "No 'develop' branch or other 'dev/*' branches found."
                log_error "Please specify explicitly: --restore-private <source-branch>"
                exit 1
            fi
        fi
    fi
...
case "$cmd" in
    ...
    --push-public)
        ...
        if [ "$remote_public_branch_count" -eq 0 ]; then
            ...
            # Clean submodule working directories before checkout to prevent conflicts
            log_info "Cleaning submodule working directories..."
            submodule_paths=$(git config -f .gitmodules --get-regexp 'submodule\..*\.path' 2>/dev/null | awk '{print $2}' || true)
            for sm_path in $submodule_paths; do
                if [ -d "$sm_path" ] && [ -n "$(ls -A "$sm_path" 2>/dev/null)" ]; then
                    # Check for uncommitted changes in submodule
                    if [ -d "$sm_path/.git" ] || [ -f "$sm_path/.git" ]; then
                        if ! git -C "$sm_path" diff --quiet HEAD 2>/dev/null; then
                            log_error "Submodule '$sm_path' has uncommitted changes."
                            log_error "Please commit or stash them before running --push-public."
                            exit 1
                        fi
                    fi
                    log_info "  Removing: $sm_path"
                    rm -rf "$sm_path"
                fi
            done
