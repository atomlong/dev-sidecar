...
- **启动时自动临时锁定（v2.2.8+）**：启动注入节点后 DS 会自动锁定延时最低的节点（因为 observatory 首轮探测完成前 leastPing 无数据、不选任何节点），约 5 分钟后自动解锁交还动态选择；若届时 observatory 仍无数据会自动延长锁定
...
- 如果锁定节点被 Stage3 热刷新删除（延迟升高或探测失败），DS 会**自动改锁剩余节点中延时最低的新节点**（v2.2.8+，此前是直接解锁），保持代理不中断
- DS 重启后锁定会自动失效（xray 进程重启后 balancer override 丢失）
- 锁定只影响**新连接**，已有连接继续走原节点完成
- observatory 首轮探测完成前手动解锁会被拒绝（balancer 无数据可选，解锁即断流）
