...
const COUNTRY_MMDB_FILENAME = 'GeoLite2-Country.mmdb'
const COUNTRY_MMDB_URL = `https://raw.githubusercontent.com/P3TERX/GeoLite.mmdb/download/${COUNTRY_MMDB_FILENAME}`
const ASN_MMDB_FILENAME = 'GeoLite2-ASN.mmdb'
const ASN_MMDB_URL = `https://raw.githubusercontent.com/P3TERX/GeoLite.mmdb/download/${ASN_MMDB_FILENAME}`
...
async function stageCountryMmdb () {
  const destPath = path.join(EXTRA_DIR, COUNTRY_MMDB_FILENAME)
  const cachePath = path.join(CACHE_DIR, `country-mmdb-${COUNTRY_MMDB_FILENAME}`)
  await download(COUNTRY_MMDB_URL, cachePath)
  fs.copyFileSync(cachePath, destPath)
  console.log(`[Country MMDB] Downloaded ${COUNTRY_MMDB_URL} -> ${destPath}`)
}

async function stageAsnMmdb () {
  const destPath = path.join(EXTRA_DIR, ASN_MMDB_FILENAME)
  const cachePath = path.join(CACHE_DIR, `asn-mmdb-${ASN_MMDB_FILENAME}`)
  await download(ASN_MMDB_URL, cachePath)
  fs.copyFileSync(cachePath, destPath)
  console.log(`[ASN MMDB] Downloaded ${ASN_MMDB_URL} -> ${destPath}`)
}
...
async function download (urlStr, dest) {
  if (fs.existsSync(dest)) {
    if ([
      `country-mmdb-${COUNTRY_MMDB_FILENAME}`,
      `asn-mmdb-${ASN_MMDB_FILENAME}`,
    ].includes(path.basename(dest))) {
